...
def process_with_gpt(openai_api_key, transcription, purpose, audience, duration, tone, additional_requirements, topic, speech_analysis):
    from openai import OpenAI  # Adjust as needed if OpenAI is directly accessible or requires specific import
    ...
    try:
        ...
        user_prompt = f"""
        Please refine this speech to be more impactful and polished:
        ...
        Provide the refined speech with:
        - **Bold** text for emphasized words
        - *Italics* for pause markers
        - Natural pacing for {duration} duration
        - Professional language (no filler words)

        Return just the refined speech text, maintaining the specified tone while optimizing for clarity and impact.
        """
        # user_prompt = f"""
        ...
        # Based on the duration , increase the speech length. 

        # Please provide your response in the following structured format:
        
        # ### Refined Speech
        # (Provide the refined version here that matches all requirements.)
        
        # ### Feedback
        # (Include detailed feedback on the original speech, covering:
        ...
        # Use markers to separate the refined speech and feedback
        # refined_marker = "### Refined Speech"
        # feedback_marker = "### Feedback"
        
        # refined_speech = ""
        # feedback = "Feedback not available."
        
        # # Find and separate sections based on markers
        # if refined_marker in full_response and feedback_marker in full_response:
        #     refined_speech = full_response.split(refined_marker)[1].split(feedback_marker)[0].strip()
        #     feedback = full_response.split(feedback_marker)[1].strip()
        # else:
        #     # Fallback if markers are not as expected
        #     parts = full_response.split("Feedback")
        #     refined_speech = parts[0].strip()
        #     feedback = parts[1].strip() if len(parts) > 1 else "Feedback not available."
        
        # return refined_speech, feedback
...
def services():
    ...
    uploaded_file = st.file_uploader("Upload your audio/video file", type=['mp3', 'wav', 'mp4'])
    ...
    if uploaded_file is not None or uploaded_document is not None:
        ...
        if st.button("Process Speech"):
            ...
            try:
                ...
                text_filepath = save_processed_data(st.session_state.session_id, 'text', refined_speech)
                ...
                """, unsafe_allow_html=True)
                # Clean the text
                cleaned_speech = re.sub(r'\*\*|\*_pause_\*|_|#', '', refined_speech)
            ...
            #     """, unsafe_allow_html=True)
                ...
                # </div>
                # """
                content = format_feedback_to_html(feedback, transcription, refined_speech)
                st.html(content)
